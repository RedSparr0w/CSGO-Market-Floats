...
handlers[Language.Client2GCEconPreviewDataBlockResponse] = function(message) {
	var itemListResponse = Protos.CMsgGCCStrike15_v2_Client2GCEconPreviewDataBlockResponse.decode(message);
	if (this.debug) {
		console.log("Received item list");
	} 
	var itemid=0;
	if (itemListResponse.iteminfo.itemid.low<0){
		var high = itemListResponse.iteminfo.itemid.high+1;
		var highval = high*4294967295;
		itemid = highval+itemListResponse.iteminfo.itemid.low+high;
	}
	else {
		var highval = itemListResponse.iteminfo.itemid.high*4294967295;
		itemid = itemListResponse.iteminfo.itemid.low+highval+itemListResponse.iteminfo.itemid.high;
	}
	var iteminfo = JSON.stringify(itemListResponse, null, 2);
	console.log(iteminfo+" - "+itemid);
  this.emit("itemList"+itemid, itemListResponse);
};

handlers[Language.PlayersProfile] = function(message) {
	var playerProfile = Protos.CMsgGCCStrike15_v2_PlayersProfile.decode(message);
	if (this.debug) {
		console.log("Received player profile");
	} 
  this.emit("playerProfile", playerProfile);
};